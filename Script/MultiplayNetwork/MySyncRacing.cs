...
public class MySyncRacing : MonoBehaviour, IPunObservable
{
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            ...
            stream.SendNext(SpawnManagerRacing.gameOver);
            stream.SendNext(SpawnManagerRacing.time);
        ...
        else
        {
            ...
            SpawnManagerRacing.otherGameOver = (bool)stream.ReceiveNext();
            SpawnManagerRacing.otherTime = (float)stream.ReceiveNext();
            SpawnManagerRacing.otherName = (string)stream.ReceiveNext();
        }
    }
}